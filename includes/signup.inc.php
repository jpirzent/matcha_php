		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{
						if (preg_match('/^[1-9][0-9]{0,15}$/', $age))
						{
							if (preg_match( '~[A-Z]~', $pwd) && preg_match( '~[a-z]~', $pwd) && preg_match( '~\d~', $pwd) && (strlen( $pwd) > 8))
							{
								...
								$sql = "INSERT INTO users (user_first, user_last, user_email, user_uid, user_pwd, user_age, user_key) VALUES (:firstn, :lastn, :email, :username, :hashpwd, :age, :hkey);";
								try
								{
									$stmt = $conn->prepare($sql);
